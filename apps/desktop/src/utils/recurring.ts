...
export async function closePreviousOccurrence(
  ...
  try {
    ...
    await db.upsertTaskToCache(updatedInstance);
...
export async function ensureActiveOccurrence(
  ...
  const now = new Date().toISOString();
  const occurrenceTitle = formatOccurrenceTitle(templateTask.title, nextDeadline);
  ...
  // Save to database
  await db.upsertTaskToCache(newInstance);
